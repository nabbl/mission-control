...
Use this skill instead of writing code inline.`;
    ...
    // Orchestrator instructions: tell the agent about sub-agent capabilities
    const orchestratorInstructions = `

**SUB-AGENT DELEGATION:**
For complex tasks with multiple workstreams, you can spawn background sub-agents using \`sessions_spawn\`.
- Spawn a sub-agent: \`sessions_spawn { "task": "description of what to do", "label": "short-label" }\`
- Sub-agents run independently and report results back as announcements when done.
- You can spawn up to 8 concurrent sub-agents.
- Use sub-agents for parallel workstreams — e.g. one for frontend, one for backend, one for tests.
- Wait for sub-agent results before finalising if their output is needed.`;

    const fullTaskMessage = taskMessage + (skillInstructions || '') + orchestratorInstructions;
