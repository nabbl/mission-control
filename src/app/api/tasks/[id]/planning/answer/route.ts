...
Always return exactly one agent — the orchestrator who will coordinate the work. For complex tasks involving multiple workstreams, the orchestrator will use sub-agents (via sessions_spawn) to delegate sub-tasks at runtime. Do NOT return multiple agents or subtasks; always return a single orchestrator agent.
...
You MUST respond with a JSON object containing a "type" field set to either "question" or "complete".`;
    ...
    if (result.type === 'complete') {
      ...
      syslog('info', 'planning', `Planning complete for task ${taskId} — ${result.agents?.length ?? 0} agents, suggested model: ${suggestedModel?.provider ?? 'default'}/${suggestedModel?.model ?? 'default'}`);
